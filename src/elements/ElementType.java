...
//    },
    ...
    public static Element createParticleByMatrix(CellularMatrix matrix, int x, int y, Vector3f Vector3f, ElementType elementType, Color color, boolean isIgnited) {
        if (matrix.isWithinBounds(x, y)) {
            Element newElement = new Particle(x, y, Vector3f, elementType, color, isIgnited);
    ...
    public static Element createParticleByMatrix(CellularMatrix matrix, int x, int y, Vector3f Vector3f, Element sourceElement) {
        if (matrix.isWithinBounds(x, y)) {
            Element newElement = new Particle(x, y, Vector3f, sourceElement);
            matrix.setElementAtIndex(x, y, newElement);
            return newElement;
        }
        return null;
