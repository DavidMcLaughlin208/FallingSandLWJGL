...
public abstract class Liquid extends Element {
    ...
    public void step(CellularMatrix matrix) {
        ...
        int smallerCount;
        Vector3f formerLocation = new Vector3f(getMatrixX(), getMatrixY(), 0);
        Vector3f lastValidLocation = new Vector3f(getMatrixX(), getMatrixY(), 0);
...
//        }
    ...
    private boolean yDidNotChange(Vector3f formerLocation) {
...
//            }
    ...
    private boolean iterateToAdditional(CellularMatrix matrix, int startingX, int startingY, int distance, Vector3f lastValid) {
        int distanceModifier = distance > 0 ? 1 : -1;
        Vector3f lastValidLocation = new Vector3f(lastValid);
    ...
    private void swapLiquidForDensities(CellularMatrix matrix, Liquid neighbor, int neighborX, int neighborY, Vector3f lastValidLocation) {
        vel.y = -62;
        if (Math.random() > 0.8f) {
            vel.x *= -1;
        }
    ...
    private void setAdjacentNeighborsFreeFalling(CellularMatrix matrix, int depth, Vector3f lastValidLocation) {
        if (depth > 0) return;
