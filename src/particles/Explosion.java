    ...
    private void iterateBetweenTwoPoints(int matrixX, int matrixY, int newX, int newY, int strength, Map<String, String> cache, CellularMatrix matrix) {
        ...
        for (int i = 0; i <= upperBound; i++) {
            ...
            if (distance < localRadius/2) {
                ...
                element.receiveHeat(matrix, 300);
                Vector2f center = new Vector2f(matrixX, matrixY);
                Vector2f newPoint = new Vector2f(currentX, currentY);
                newPoint.sub(center).normalize();
                matrix.particalizeByMatrix(currentX, currentY, new Vector3f(newPoint.x * radius * 5, newPoint.y  * radius * 5, 0));
                if (Math.random() > .8) {
                    break;
                }
            }
