package box2dfs.douglaspeucker;
...
public class RecursiveApproach {
    ...
    private Vector2f sum(Vector2f v1, Vector2f v2) {
        return new Vector2f(v1.x + v2.x, v1.y + v2.y);
    }

    private Vector2f diff(Vector2f v1, Vector2f v2) {
        return new Vector2f(v1.x - v2.x, v1.y - v2.y);
    }

    private Vector2f prod(Vector2f v1, Vector2f v2) {
        return new Vector2f(v1.x * v2.x, v1.y * v2.y);
    }
    ...
    private float norm2(Vector2f v) {
        return (v.x * v.x) + (v.y * v.y);
    }

    private float norm(Vector2f v) {
        return (float) Math.sqrt(norm2(v));
    }

    private float d2(Vector2f v1, Vector2f v2) {
        return norm2(diff(v1,v2));
    }

    private float d(Vector2f v1, Vector2f v2) {
        return norm(diff(v1,v2));
    }

    public void simplifyDP(float tolerance, Vector2f[] vertices, int startIndex, int endIndex, int[] markerBuffer) {
        ...
        Vector2f[] segment = new Vector2f[] {vertices[startIndex], vertices[endIndex]};  // segment from v[j] to v[k]
        Vector2f direction = diff(segment[1], segment[0]);   // segment direction vector
        float squaredSegmentLength = norm2(direction);     // segment length squared
        // test each vertex v[i] for max distance from S
        // compute using the Feb 2001 Algorithm's dist_Point_to_Segment()
        // Note: this works in any dimension (2D, 3D, ...)
        Vector2f w;           // vector
        Vector2f Pb;                // point, base of perpendicular from v[i] to S
        ...
        for (int i=startIndex + 1; i < endIndex; i++) {
            ...
            if ( cw <= 0 ) {
                ...
                Pb = new Vector2f(segment[0].x+b*direction.x, segment[0].y+b*direction.y);
    ...
    public List<Vector2f> simplify(List<Vector2f> vertices, float tolerance) {
        int n = vertices.size();
        Vector2f[] savedVerts = new Vector2f[n];
        int i, k, m, pv;               // misc counters
        float tol2 = tolerance * tolerance;          // tolerance squared
        Vector2f[] vertexBuffer = new Vector2f[n];                       // vertex buffer, points
